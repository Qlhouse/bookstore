...
def uploadProduct(rootDir):
    ...
    classifyOptions = driver.find_element(
        By.XPATH,
    ...
    def uploadBriefImages(imgPath):
        # 点击 '选择图片'
        chooseImgButton = driver.find_element(
            By.XPATH, '//*[@id="list"]/div[1]/div[2]/div[5]/div/div[1]/span/button'
        )
        chooseImgButton.click()

        # 传输图片
        uploadFileElement = driver.find_element(By.ID, "we7resourceFile")
        ...
        selectImage = driver.find_element(
            By.XPATH, '//*[@id="image"]/div/div[2]/div[1]'
        )
        selectImage.click()

        # 点击 '确定'，退出对话框
        sureButton = driver.find_element(
            By.XPATH, '//*[@id="material-Modal"]/div/div/div[3]/button[1]'
        )
        sureButton.click()
        time.sleep(5)

    # 上传商品简介图
    # briefImageDir = os.path.join(rootDir + "synopsisImgDir")
    ...
    # Check '详情显示首图'
    showFirtImgInDetails = driver.find_element(By.NAME, "thumb_first")
    ...
    # Set sell price
    setSellPrice = driver.find_element(By.ID, "marketprice")
    setSellPrice.send_keys(data["售价"])

    # Set original price
    setOriginalPrice = driver.find_element(By.ID, "productprice")
    setOriginalPrice.send_keys(data["原价"])

    time.sleep(10)

    # Check "支持货到付款"
    cashOnDelivery = driver.find_element(
        By.XPATH, '//*[@id="list"]/div[2]/div[2]/div[7]/div/label[1]/input'
    )
    cashOnDelivery.click()

    # 设置运费模板
    freightFormwork = driver.find_element(By.ID, "s2id_dispatchid")
    freightFormwork.click()
    time.sleep(5)

    # Locate the choosed classification
    deliveryOption = driver.find_element(By.XPATH, "//div[text()='满49元免费送上门']")
    deliveryOption.click()

    # Check "上架"
    releaseToMarket = driver.find_element(
        By.XPATH, '//*[@id="list"]/div[2]/div[2]/div[10]/div/label[2]/input'
    )
    releaseToMarket.click()

    # 下一步
    # nextStep = driver.find_element_by_id("next")
    nextStep = driver.find_element(By.XPATH, '//*[@id="next"]')
    nextStep.click()
    time.sleep(5)

    # 填写 '规格/库存' 信息
    # Fill 编码
    productEncoding = driver.find_element(By.ID, "goodssn")
    productEncoding.send_keys(data["编码"])

    # Fill 条编
    productBarcode = driver.find_element(By.ID, "productsn")
    productBarcode.send_keys(data["条码"])

    # Fill 库存
    goodsInStock = driver.find_element(By.ID, "total")
    goodsInStock.send_keys(data["库存"])

    # Check '显示库存'
    showGoodsInStock = driver.find_element(By.ID, "showtotal")
    showGoodsInStock.click()

    # 下一步
    nextStep = driver.find_element(By.ID, "next")
    ...
    # 商品副标题
    goodShortTitle = driver.find_element(By.ID, "subtitle")
    goodShortTitle.send_keys(data["商品副标题"])
    # 商品短标题
    goodShortTitle = driver.find_element(By.NAME, "shorttitle")
    ...
    def uploadProductDetailImg(imgPath):
        # Upload image
        # 打开插入图片按钮
        uploadImgButton = driver.find_element(By.ID, "edui144_body")
    ...
    # def uploadContentsText(htmlText):
    #     driver.switch_to.frame("ueditor_0")
    #     time.sleep(3)
    #     # bodyElement = driver.find_element(By.XPATH, "/html/body")
    #     driver.execute_script(
    #         f"""
    #         function getElementByXpath(path) {
    #         return document.evaluate(path, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
    #     }
    #         getElementByXpath('/html/body').value = {htmlText}"""
    #     )
    #     time.sleep(5)
    #     driver.switch_to.default_content()

    # def inputText():
    #     htmlLocator = driver.find_element(By.ID, "edui4_body")
    #     htmlLocator.click()
    ...
    for file in os.listdir(detailImageDir):
        if file.endswith(".jpg"):
            uploadProductDetailImg(os.path.abspath(file))
        # elif file.endswith(".txt"):
        #     with open("contents.txt", "r") as htmlText:
        #         uploadContentsText(htmlText)
    ...
    # Save product
    saveProduct = driver.find_element(
        By.XPATH, "/html/body/div[6]/div[2]/div/div/form/div/div[4]/input"
    )
    saveProduct.click()

    uploadSuccess = r"D:\bookStore\uploadProductToWebsite\uploadSuccess.txt"
    with open(uploadSuccess, "a+") as fh:
        fh.write(os.path.basename(rootDir) + "\n")

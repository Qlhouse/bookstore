...
destinationRootPath = r"D:\bookStore\makeProduct"
imgLinksFile = r"D:\bookStore\makeProduct\productDetial.html"
successTone = os.path.abspath("D:\\bookStore\\uploadProductToWebsite\\successTone.wav")
errorTone = os.path.abspath("D:\\bookStore\\uploadProductToWebsite\\errorTone.wav")

master = tk.Tk(className="移动图片-京东版")
...
def downloadImg():
    ...
    imgSite = imgSite.replace("n5", "imgzone", 1)
...
def moveBriefImgs():
    ...
    originalPath = r"C:\Users\xq127\Downloads\pictures\synopsisImgDir"
    destinationPath = os.path.join(destinationRootPath, targetDir, "synopsisImgDir")
    ...
    if any(os.scandir(originalPath)):
        emptyTargetDir()
        ...
        briefImages = os.listdir(originalPath)

        try:
            for img in sorted(briefImages, key=os.path.getctime):
                shutil.move(os.path.abspath(img), destinationPath)
            
            # playsound(successTone)
            winsound.PlaySound(successTone, winsound.SND_PURGE)
        except:
            print('Wrong!')
            # playsound(errorTone)
            winsound.PlaySound(errorTone, winsound.SND_PURGE)
...
def downloadConcatenateImgs():
    ...
    # imgLinksFile = r"D:\bookStore\makeProduct\productDetial.html"
    destinationPath = os.path.join(destinationRootPath, targetDir, "detailImageDir")

    # Remove original files in destinationPath
    for filename in os.listdir(destinationPath):
        file_path = os.path.join(destinationPath, filename)
        try:
            if os.path.isfile(file_path) or os.path.islink(file_path):
    ...
    # else:
    #     contentsText = os.path.join(destinationPath, "contents.txt")
    #     with open(contentsText, "w", encoding="utf-8") as fh:
